-- ============================================
-- DealerCenter Integration - Database Migration
-- ============================================
-- Run this migration on production database BEFORE deploying API
...
CREATE TABLE IF NOT EXISTS dealercenter_dealers (
  ...
  -- Subscription Info
  tier_id text REFERENCES subscription_tiers(tier_id),
...
-- NOTE: You may need to manually update external_listings source constraint
-- to allow 'dealercenter' source if it has a CHECK constraint.
-- Check current constraint: 
-- SELECT pg_get_constraintdef(oid) FROM pg_constraint WHERE conname = 'external_listings_source_check';
-- Then add 'dealercenter' to the list manually if needed.
