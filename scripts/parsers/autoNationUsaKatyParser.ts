...
async function fetchListings(): Promise<VehicleData[]> {
  ...
  const baseUrl = 'https://www.autonationusa.com/all-inventory/index.htm?geoZip=&geoRadius=0&accountId=autonationusakaty';
  ...
  try {
    ...
    for (let pageNum = 0; pageNum < pagesToParse; pageNum++) {
      ...
      try {
        ...
        const detailFetchTasks = pageVehicles.map((v) => async () => {
          try {
            ...
            // Use UUID as VIN if no VIN extracted (for Katy site)
            ...
            const vehicleUrl = v.detailUrl 
              ...
              : `https://www.autonationusa.com/all-inventory/index.htm?geoZip=&geoRadius=0&accountId=autonationusakaty`;
...
export async function syncAutoNationUsaKaty() {
  console.log('🚗 Starting AutoNation USA Katy sync...');
  ...
  try {
    ...
    console.error('Error in AutoNation USA Katy sync:', error);
    throw error;
  }
}
