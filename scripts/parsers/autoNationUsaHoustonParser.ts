...
async function fetchListings(): Promise<VehicleData[]> {
  ...
  const baseUrl = 'https://www.autonationusa.com/all-inventory/index.htm?geoZip=&geoRadius=0&accountId=autonationusahouston';
...
export async function syncAutoNationUsaHouston() {
  console.log('🚗 Starting AutoNation USA Houston sync...');
  ...
  try {
    ...
    console.error('Error in AutoNation USA Houston sync:', error);
    throw error;
  }
}
