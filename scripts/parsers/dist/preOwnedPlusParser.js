...
async function fetchImageFromDetailPage(url, browser) {
    let page = null;
    try {
        ...
        const imageUrl = await page.evaluate(() => {
            ...
            if (promaxImages.length > 0) {
                const firstImage = promaxImages[0];
            ...
            if (vehicleImages.length > 0) {
                vehicleImages.sort((a, b) => {
                    const aSize = (a.naturalWidth || a.width) *
                        (a.naturalHeight || a.height);
                    const bSize = (b.naturalWidth || b.width) *
    ...
    try {
        ...
        const vehicleData = await page.evaluate((baseUrl) => {
            ...
            links.forEach((link) => {
                ...
                const containerText = (container === null || container === void 0 ? void 0 : container.textContent) || '';
                // Extract title
...
async function downloadAndUploadImage(imageUrl, listingId, 
// eslint-disable-next-line @typescript-eslint/no-explicit-any
supabase) {
    var _a;
    try {
        ...
        const ext = ((_a = imageUrl.split('.').pop()) === null || _a === void 0 ? void 0 : _a.split('?')[0]) || 'jpg';
...
supabase) {
    ...
    const { data: cityData } = await supabase
        ...
        .ilike('name', CITY)
        .single();
    const cityId = cityData ? cityData.id : null;
    return {
        stateId,
        cityId
    };
...
async function syncListings(listings, supabaseUrl, supabaseKey) {
    ...
    for (const listing of listings) {
        try {
            ...
            let uploadedImageUrl = (existing === null || existing === void 0 ? void 0 : existing.image_url) || null;
            ...
            if (existing) {
                ...
                const { error } = await supabase
                    .from('external_listings')
                    .update(Object.assign(Object.assign({}, listingData), { updated_at: currentTime }))
                    .eq('id', existing.id);
                if (error) {
...
// Main sync function
async function syncPreOwnedPlus(supabaseUrl, supabaseKey) {
