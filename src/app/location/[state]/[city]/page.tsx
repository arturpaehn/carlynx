...
export default function CityPage() {
  ...
  const [stateData, setStateData] = useState<{ id: number; name: string; code: string } | null>(null);

  // Decode and format names
  const stateName = stateParam ? decodeURIComponent(stateParam).toLowerCase() : '';
  const cityName = cityParam ? decodeURIComponent(cityParam).toLowerCase() : '';
    ...
    const fetchListings = async () => {
      ...
      try {
        ...
        const { data: ownListings, error: ownError } = await supabase
          ...
          .ilike('city_name', `%${cityName.replace(/-/g, ' ')}%`)
    ...
    return (
      <main className="min-h-screen bg-gradient-to-br from-orange-50 via-yellow-50 to-orange-100 pt-header">
        <div className="max-w-7xl mx-auto px-4 py-12 text-center">
          ...
          <p className="text-gray-700">Loading {cityDisplay} listings...</p>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-gradient-to-br from-orange-50 via-yellow-50 to-orange-100 pt-header">
      <div className="max-w-7xl mx-auto px-4 py-8 sm:py-12">
        ...
        <nav className="text-sm text-gray-600 mb-4">
          ...
          <span className="mx-2">/</span>
          <span className="text-gray-900 font-medium">{cityDisplay}, {stateDisplay}</span>
        </nav>

        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-2">
            Used Cars & Motorcycles for Sale in {cityDisplay}, {stateData?.name || stateDisplay}
          </h1>
          ...
          <div className="mt-4 bg-white/70 backdrop-blur-sm rounded-lg p-4 border border-orange-100">
            <p className="text-gray-700">
              Browse the best selection of used vehicles in {cityDisplay}, {stateData?.name}. 
              Find cars, trucks, SUVs, and motorcycles from local sellers and trusted dealers. 
        ...
        {listings.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-600 mb-4">No listings found in {cityDisplay}.</p>
            <Link
              href="/search-results"
              className="text-orange-600 hover:text-orange-700 underline"
            >
                ...
                <div className="relative h-48 bg-gray-200">
                  {listing.image_url ? (
                    <Image
                      ...
                      alt={`${listing.year} ${listing.title} in ${cityDisplay}`}
                      fill
